...
#define X_PRC_SENSORS_STORE_ENGINE(...) \
  ...
  X_FIELD (double, temperature_OIN, ##__VA_ARGS__) \
  X_FIELD (double, temperature_EIN, ##__VA_ARGS__)
#endif
...
#define X_PRC_SENSORS_STORE_LOX(...) \
  ...
  X_proc_FIELD(double, pressure_OTA, ##__VA_ARGS__) \
  ...
  X_FIELD(double, FLS, ##__VA_ARGS__)
#endif
